...
def itterationApprox(ksi):
    return np.log(1 / EPS) / np.sqrt(ksi)
...
def Relax(x, y, step_x, step_y, iter, omega):
    k = 0
    ...
    while (
        ...
        for i in range(1, len(x) - 1):
            for j in range(1, len(y) - 1):
                ...
                term3 = (
                    p(x[i] - step_x / 2, y[j])
                    * (previous_U[i][j] - current_U[i - 1][j])
                ) / delta_x_sq
                ...
                term5 = (
                    q(x[i], y[j] - step_y / 2)
                    * (previous_U[i][j] - current_U[i][j - 1])
                ) / delta_y_sq
...
array_simple_itteration_matrix, amountItter = Relax(
    x_i, y_i, step_x, step_y, itterationApprox(ksi), omegaOpt(spectr)
)
